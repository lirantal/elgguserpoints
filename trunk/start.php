<?php
    ...
    function userpoints_init() {        
        ...
        add_widget_type('toppoints', $toppoints_name, elgg_echo('elggx_userpoints:widget:toppoints:info'));

        // Hooks for awarding points
        register_plugin_hook('permissions_check', 'all', 'elggx_userpoints_permissions_check');
        register_plugin_hook('action', 'invitefriends/invite', 'elggx_userpoints_invite');
        register_plugin_hook('action', 'register', 'elggx_userpoints_register');
        register_plugin_hook('action', 'email/confirm', 'elggx_userpoints_validate');
        register_plugin_hook('action', 'siteaccess/confirm', 'elggx_userpoints_validate');
        register_plugin_hook('action', 'recommendations/new', 'elggx_userpoints_recommendations');
        register_plugin_hook('action', 'recommendations/approve', 'elggx_userpoints_recommendations');
        register_elgg_event_handler('login','user','elggx_userpoints_login');
        register_elgg_event_handler('create','object', 'elggx_userpoints_object');
        register_elgg_event_handler('delete','object', 'elggx_userpoints_object');
        register_elgg_event_handler('delete','entity', 'elggx_userpoints_object');
        register_elgg_event_handler('create','annotation','elggx_userpoints_annotate_create');
        register_elgg_event_handler('create','group','elggx_userpoints_group');
        register_elgg_event_handler('delete','group','elggx_userpoints_group');
        register_elgg_event_handler('upload','tp_album','elggx_userpoints_tidypics');
        register_elgg_event_handler('profileupdate','user','elggx_userpoints_profile');

        // Upgrage settings
        $oldversion = get_plugin_setting('version', 'userpoints');
        $newversion = get_plugin_setting('version', 'elggx_userpoints');

        // Check if we need to run an upgrade
        if ($oldversion && !$newversion) {

            // Update plugin settings
            $plugin = find_plugin_settings('userpoints');

            set_plugin_setting('upperplural', $plugin->upperplural, 'elggx_userpoints');
            set_plugin_setting('lowerplural', $plugin->lowerplural, 'elggx_userpoints');
            set_plugin_setting('lowersingular', $plugin->lowersingular, 'elggx_userpoints');
            set_plugin_setting('moderate', $plugin->moderate, 'elggx_userpoints');
            set_plugin_setting('subtract', $plugin->subtract, 'elggx_userpoints');
            set_plugin_setting('displaymessage', $plugin->displaymessage, 'elggx_userpoints');
            set_plugin_setting('mypoints_name', $plugin->mypoints_name, 'elggx_userpoints');
            set_plugin_setting('toppoints_name', $plugin->toppoints_name, 'elggx_userpoints');
            set_plugin_setting('numtoppoint', $plugin->numtoppoint, 'elggx_userpoints');
            set_plugin_setting('profile_display', $plugin->profile_display, 'elggx_userpoints');
            set_plugin_setting('delete', $plugin->delete, 'elggx_userpoints');
            set_plugin_setting('expire_after', $plugin->expire_after, 'elggx_userpoints');


            // Update point settings
            $pointssettings = find_plugin_settings('userpoints_standard');

            set_plugin_setting('blog', $pointssettings->blog, 'elggx_userpoints');
            set_plugin_setting('group', $pointssettings->group, 'elggx_userpoints');
            set_plugin_setting('profileupdate', $pointssettings->profileupdate, 'elggx_userpoints');
            set_plugin_setting('page_top', $pointssettings->page_top, 'elggx_userpoints');
            set_plugin_setting('generic_comment', $pointssettings->generic_comment, 'elggx_userpoints');
            set_plugin_setting('riverpost', $pointssettings->riverpost, 'elggx_userpoints');
            set_plugin_setting('thewire', $pointssettings->thewire, 'elggx_userpoints');
            set_plugin_setting('upload_photo', $pointssettings->upload_photo, 'elggx_userpoints');
            set_plugin_setting('poll', $pointssettings->poll, 'elggx_userpoints');
            set_plugin_setting('pollvote', $pointssettings->pollvote, 'elggx_userpoints');
            set_plugin_setting('phototag', $pointssettings->phototag, 'elggx_userpoints');
            set_plugin_setting('group_topic_post', $pointssettings->group_topic_post, 'elggx_userpoints');
            set_plugin_setting('login', $pointssettings->login, 'elggx_userpoints');
            set_plugin_setting('delete', $pointssettings->delete, 'elggx_userpoints');
            set_plugin_setting('invite', $pointssettings->invite, 'elggx_userpoints');
            set_plugin_setting('verify_email', $pointssettings->verify_email, 'elggx_userpoints');
            set_plugin_setting('require_registration', $pointssettings->require_registration, 'elggx_userpoints');
            set_plugin_setting('expire_invite', $pointssettings->expire_invite, 'elggx_userpoints');

            // Set new version
            set_plugin_setting('version', '1.2.3', 'elggx_userpoints');
        }
    }
    ...
    function userpoints_add($user_guid, $points, $description, $type=null, $guid=null) {
        ...
        if (!trigger_plugin_hook('userpoints:add', $userpoint->type, array('entity' => $userpoint), true)) {
            $userpoint->delete();
            return(false);
        }
        ...
        if (get_plugin_setting('displaymessage') && $type != 'admin' && $user_guid == $_SESSION['user']->guid) {
            $message = get_plugin_setting('moderate') ? 'elggx_userpoints:pending_message' : 'elggx_userpoints:awarded_message';
    ...
    function userpoints_adminmenu()
    {
        ...
        if (get_context() == 'admin' && isadminloggedin()) {
            add_submenu_item(elgg_echo('elggx_userpoints:userpoints'), $CONFIG->url . "mod/elggx_userpoints/admin.php");
        }
    }
    ...
    function userpoints_update_user($guid, $points) {
        ...
        if (!trigger_plugin_hook('userpoints:update', 'object', array('entity' => $user), true)) {
            $user->userpoints_points = $user->userpoints_points - $points;
            return(false);
        }
    }
    ...
    function elggx_userpoints_permissions_check($hook_name, $entity_type, $return_value, $parameters) {
        if (get_context() == 'userpoints_access') {
            return true;
        }
    }
    ...
    function elggx_userpoints_recommendations($hook, $action) {
        ...
        $approval = (int) get_plugin_setting('recommendations_approve');
        $points = (int) get_plugin_setting('recommendation');
    ...
    function elggx_userpoints_tidypics($event, $type, $object) {
        if ($points = get_plugin_setting('upload_photo')) {
    ...
    function elggx_userpoints_login() {
        ...
        if ($diff > get_plugin_setting('login_threshold')) {
            ...
            $s = (int) get_plugin_setting('login_interval') * 86400;
    ...
    function elggx_userpoints_register() {
        ...
        if (is_plugin_enabled('uservalidationbyemail') || is_plugin_enabled('siteaccess')) {
            return(true);
        }

        if (is_plugin_enabled('siteaccess') && get_plugin_setting('autoactivate', 'siteaccess') != 'yes') {
            return(true);
        }
    ...
    function elggx_userpoints_contact_importer($friend_guid, $email) {
        ...
        if (!$points = get_plugin_setting('invite')) {
            return true;
        }
    ...
    function elggx_userpoints_invite() {
        ...
        if (sizeof($emails)) {
            foreach($emails as $email) {
                ...
                if (get_plugin_setting('verify_email') && !elggx_userpoints_validEmail($email)) {
                    continue;
                }

                if ((int)get_plugin_setting('require_registration')) {
                    if (!elggx_userpoints_invite_status($_SESSION['user']->guid, $email)) {
                        ...
                        if (is_plugin_enabled('expirationdate') && $expire = (int)get_plugin_setting('expire_invite')) {
                            $ts = time() + $expire;
                            expirationdate_set($userpoint->guid, date('Y-m-d H:i:s', $ts), false);
                        }
                    }
    ...
    register_action("elggx_userpoints/settings", false, $CONFIG->pluginspath . "elggx_userpoints/actions/settings.php", true);
    register_action("elggx_userpoints/delete",false,$CONFIG->pluginspath . "elggx_userpoints/actions/delete.php", true);
    register_action("elggx_userpoints/moderate",false,$CONFIG->pluginspath . "elggx_userpoints/actions/moderate.php", true);
    register_action("elggx_userpoints/add",false,$CONFIG->pluginspath . "elggx_userpoints/actions/add.php", true);
    register_action("elggx_userpoints/reset",false,$CONFIG->pluginspath . "elggx_userpoints/actions/reset.php", true);
